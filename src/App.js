...
class App extends Component {
 ...
 componentDidMount() {
   axios.get('/api/saved-horo').then( res => {
   ...
   editMood=(id, newMood) => {
     ...
     axios.put(`/api/saved-horo/${id}`, body)
     .then(res => {
       this.setState({savedHoro: res.data})
     })
     .catch(err => console.log(err));
   ...
   releaseHoro = (id) => {
     axios.delete(`/api/saved-horo/${id}`)
    .then(res => {
...
render() {
  ...
  //console.log(this.state.savedHoro)

  return (    
       <div className="App">
       <Header />
       <Sections
        catchFn={this.state.savedHoro}/>
      <Horolist
